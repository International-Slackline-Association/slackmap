...
import { ConvertKeysToInterface, TransformerParams } from 'core/db/types';
import {
  TABLE_NAME,
  chunkArray,
  composeKey,
...
const { key, attrsToItem, itemToAttrs, keyFields } = transformUtils<
  DDBMapFeatureEditorItem,
  DDBMapFeatureEditorAttrs,
  typeof keysUsed
>(keyUtils);

export const getFeatureEditor = async (
  featureId: string,
...
export const deleteFeatureEditor = async (
  featureId: string,
  featureType: MapFeatureType,
  userId: string,
) => {
  return ddb.send(
    new DeleteCommand({ TableName: TABLE_NAME, Key: key({ featureId, featureType, userId }) }),
  );
};
