...
import { DDBMapFeatureChangelogAttrs, DDBMapFeatureChangelogItem } from './types';
...
export const getMultipleFeatureChangelog = async (
  ...
  const allKeys = chunkArray(
    changelogs.map((c) =>
      key({ featureId: c.featureId, featureType: c.featureType, date: c.date }),
    ),
    100,
  );
...
export const deleteFeatureChangelog = async (
  featureId: string,
  featureType: MapFeatureType,
  date: string,
) => {
  return ddb.send(
    new DeleteCommand({ TableName: TABLE_NAME, Key: key({ featureId, featureType, date }) }),
  );
};

export const deleteAllFeatureChangelogs = async (
  featureId: string,
  featureType: MapFeatureType,
) => {
