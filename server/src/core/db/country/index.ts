...
const typeSafeCheck = <
  T extends TransformerParams<
    Omit<DDBCountryItem, keyof DDBCountryAttrs>,
...
const keyUtils = typeSafeCheck({
  ...
  GSI2_SK: {
    ...
    destruct: (key) => ({
      featureType: key.startsWith('featureType')
        ? (destructKey(key, 1) as MapFeatureType)
        : undefined,
...
const { attrsToItem, keyFields } = transformUtils<DDBCountryItem, DDBCountryAttrs, typeof keysUsed>(
  keyUtils,
);
